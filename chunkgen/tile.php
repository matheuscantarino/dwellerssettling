<?php
     ...
     while ($register = mysqli_fetch_array($result)){
          ...
          $dweller_current = 1;
               ...
               $dwellerpossql = "UPDATE dwellers SET dweller_xpos = '{$dweller_xpos}' WHERE dweller_id = '{$dweller_current}'";
                    if ($conn->query($dwellerpossql) === TRUE) {
                         echo "Record updated successfully";
                         } else {
                         echo "Error updating record: " . $conn->error;
                    }
               $dwellerpossql = "UPDATE dwellers SET dweller_ypos = '{$dweller_ypos}' WHERE dweller_id = '{$dweller_current}'";
                    if ($conn->query($dwellerpossql) === TRUE) {
                         echo "Record updated successfully";
                         } else {
                         echo "Error updating record: " . $conn->error;
                    }
               $dwellerpossql = "UPDATE dwellers SET dweller_current_pos = '{$current_pos}' WHERE dweller_id = '{$dweller_current}' ";
                    if ($conn->query($dwellerpossql) === TRUE) {
                         echo "Record updated successfully";
                         } else {
                         echo "Error updating record: " . $conn->error;
